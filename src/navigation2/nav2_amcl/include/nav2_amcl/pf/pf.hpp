...
typedef struct _pf_sample_set_t
{
  ...
  // 当前粒子簇的数量（从1开始，不是从0开始）和最大粒子簇数量，就是有多少峰值
  ...
  pf_vector_t mean;
  pf_matrix_t cov;
  // 每一个粒子的位姿pose在x,y方向上的值分别减去平均值mean_x,mean_y,得到的差值与dist_threshold比较，如果差值大于dist_threshold那就显示没收敛，粒子还处于发散的状态中
  // 收敛就是1，不收敛就是0
  int converged;
} pf_sample_set_t;
...
typedef struct _pf_t
{
  ...
  double w_slow, w_fast, total_w, w_diff, lost_w, poor_w, lost_m, poor_m, poor_wdiff, total_w_max, x_t[9], y_t[9], th_t[9], loop, pose_x, pose_y, poor_th;
...
int valid_map_xy(map_t *map, double xx, double yy);
