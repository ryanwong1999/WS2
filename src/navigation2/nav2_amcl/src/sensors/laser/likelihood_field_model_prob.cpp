...
LikelihoodFieldModelProb::sensorFunction(LaserData * data, pf_sample_set_t * set)
{
  ...
  double z, pz;
  double log_p;
  double obs_range = 0.0, obs_bearing = 0.0, total_weight = 0.0;
  double obs_r_temp, obs_r_max;
  pf_sample_t * sample;
  pf_vector_t pose;
  pf_vector_t hit;
  ...
  for (j = 0; j < set->sample_count; j++) {
    ...
    for (i = 0; i < data->range_count; i += step) {
      // following added by wyj
      // 根据传感器数据中的一系列距离值，获取满足一定条件的最大距离值obs_r_max,并记录对应的索引ik，
      // 然后根据该索引对应的距离值data->range[ik][0]赋值给obs_range，将角度值data->range[ik][1]赋给obs_bearing
      ik = 0;
      obs_r_max = 0;

      for (k = 0; k < step; k ++) {
        obs_r_temp = data->ranges[i+k][0];
	      if(obs_r_temp >= data->range_max)
          continue;
      	if(obs_r_temp != obs_r_temp)
          continue;
      	if(obs_r_temp < 0.3)
          continue;

        if(obs_r_temp > obs_r_max)
          obs_r_max = obs_r_temp;
        ik = i+k;
      }

      obs_range = 99;
      if(ik > 0) {
        obs_range = data->ranges[ik][0];
        obs_bearing = data->ranges[ik][1];
      }
      // End of wyj added
