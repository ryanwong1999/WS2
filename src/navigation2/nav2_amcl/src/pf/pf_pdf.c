...
// 概率密度函数。主要是完成创建和生成一个粒子（x y theta），这个值是在pdf->x(均值)附近的
#include <assert.h>
#include <math.h>
#include <stdlib.h>
#include <string.h>
...
// 创建一个高斯概率密度函数 有期望、协方差、还有分解后的矩阵、再产生一个随机值
...
// 根据一个高斯分布，生成一个粒子（x y theta），这个值是在pdf->x附近的
pf_vector_t pf_pdf_gaussian_sample(pf_pdf_gaussian_t * pdf)
{
  ...
  for (i = 0; i < 3; i++) {
    ...
    // pf_ran_gaussian函数实现了一个标准的高斯分布函数，其接收sigma参数。即生成一个高斯(正态分布)随机数
    // 就是在pdf->x附近的值
